...
const Header: React.FC<HeaderProps> = ({ isAdmin, onLoginClick, onLogoutClick, isDarkMode, toggleTheme }) => {
  const [isMobileMenuOpen, setIsMobileMenuOpen] = useState(false);

  const toggleMenu = () => setIsMobileMenuOpen(!isMobileMenuOpen);

  // Dynamic Styles
  const headerBg = isDarkMode ? 'bg-deep-teal/50 border-off-white/10' : 'bg-light-bg/90 border-deep-teal/10 shadow-sm';
        ...
        <div className="flex justify-between items-center h-20">
          ...
          <div className="flex-shrink-0 flex items-center gap-3">
            ...
            <div className="flex flex-col">
              <a href="#" className={`font-display font-bold text-2xl tracking-wider leading-none transition-colors ${logoTextMain}`}>
                BLUEHOUSE
              </a>
          ...
          <nav className="hidden md:flex space-x-12 items-center">
            {['Collection', 'About', 'Process', 'Journal'].map((item) => (
              <a 
                key={item} 
        ...
        <div className="flex flex-col h-full pt-20 px-8 space-y-8">
           {['Collection', 'About', 'Process', 'Journal'].map((item) => (
            ...
            <div className={`border-t pt-8 mt-auto mb-8 ${isDarkMode ? 'border-off-white/10' : 'border-deep-teal/10'}`}>
               ...
               <button className={`w-full text-left ${textColor}`}>Cart (1)</button>
            </div>
        </div>
      </div>
    </header>
